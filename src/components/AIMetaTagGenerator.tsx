...
import { Type, AlignLeft, Sparkles, Copy, CheckCircle2, AlertCircle, Link2, Save } from 'lucide-react';
...
export default function AIMetaTagGenerator({ setIsLoading }: { setIsLoading: (val: boolean) => void }) {
  const [selectedTask, setSelectedTask] = useState('Pembuatan Q2 Meta Tags');
  const [topic, setTopic] = useState('AI SEO Tools');
  const [description, setDescription] = useState('A comprehensive guide covering strategies, tools, and best practices to rank higher using AI.');
  
  // Render in Post-Generation state by default
  const [result, setResult] = useState<GeneratedTags | null>({
    title: 'AI SEO Tools - The Ultimate Guide to Ranking Higher in 2024',
    description: 'Learn everything you need to know about AI SEO Tools. This comprehensive guide covers strategies, tools, and best practices to rank higher in search engines today.'
  });
  ...
  const handleGenerate = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      setResult({
        title: `${topic} - The Ultimate Guide to SEO Tools`,
        description: `Learn everything you need to know about ${topic}. This comprehensive guide covers strategies, tools, and best practices to rank higher in search engines today.`
      });
  ...
  const getLengthColor = (length: number, max: number, min: number) => {
    ...
    if (length < min) return 'text-brand-yellow'; // too short
    ...
    <div className="space-y-6 max-w-4xl mx-auto">
      ...
      <div className="bg-[var(--bg-primary)] p-4 md:p-6 rounded-2xl border border-[var(--border-color)] shadow-sm flex flex-col sm:flex-row sm:items-center gap-4">
        ...
        <div className="flex-1 relative">
           ...
           >
             <option value="Pembuatan Q2 Meta Tags">Pembuatan Q2 Meta Tags</option>
        ...
        >
          ...
          <div className="space-y-3">
            ...
            <div className="relative group">
              <div className="w-full p-4 pr-14 border border-[var(--border-color)] rounded-xl bg-[var(--bg-secondary)] text-lg text-blue-600 dark:text-blue-400 font-medium break-words cursor-text select-all">
                {result.title}
              </div>
              <button
            ...
            <div className="w-full bg-[var(--bg-secondary)] rounded-full h-1.5 overflow-hidden">
               <div className={`h-full rounded-full transition-all duration-500 ${result.title.length <= 60 ? 'bg-green-500' : 'bg-red-500'}`} style={{ width: `${Math.min((result.title.length / 60) * 100, 100)}%` }} />
            </div>
          </div>
          ...
          <div className="space-y-3 pt-4 border-t border-[var(--border-color)]">
            ...
            <div className="relative group">
              <div className="w-full p-4 pr-14 border border-[var(--border-color)] rounded-xl bg-[var(--bg-secondary)] text-base text-[var(--text-secondary)] break-words cursor-text select-all min-h-[100px]">
                {result.description}
              </div>
              <button
            ...
            <div className="w-full bg-[var(--bg-secondary)] rounded-full h-1.5 overflow-hidden">
               <div className={`h-full rounded-full transition-all duration-500 ${result.description.length <= 160 ? 'bg-green-500' : 'bg-red-500'}`} style={{ width: `${Math.min((result.description.length / 160) * 100, 100)}%` }} />
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4 pt-4 border-t border-[var(--border-color)]">
             <div className="flex-1 p-4 bg-brand-yellow/10 border border-brand-yellow/20 rounded-xl text-sm text-[var(--text-primary)] flex items-start">
                ...
                <p>Pratinjau Google SERP terlihat bagus. Tag yang dihasilkan telah dioptimalkan untuk panjang dan CTR.</p>
             </div>
             ...
             <button
               ...
               className={`w-full sm:w-auto flex-shrink-0 px-8 py-4 font-semibold rounded-xl transition-colors shadow-sm flex items-center justify-center ${
                 isSaved 
                   ...
                   : 'bg-[var(--bg-primary)] text-[var(--text-primary)] border-2 border-brand-yellow hover:bg-brand-yellow/10'
               }`}
             >
               {isSaved ? (
                 <>
                    <CheckCircle2 className="w-5 h-5 mr-2" />
                    Tag disematkan ke tugas!
                 </>
               ) : (
                 <>
